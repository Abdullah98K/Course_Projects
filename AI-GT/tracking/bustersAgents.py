...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Best Action that minimizes the distace for the given key
        Min = 1000000
        BestAction = None

        for Action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, Action)
            if self.distancer.getDistance(successorPosition, LikleyKey) < Min:
                Min = self.distancer.getDistance(successorPosition, LikleyKey)
                BestAction = Action
