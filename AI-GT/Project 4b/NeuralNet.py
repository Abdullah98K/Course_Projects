...
class Perceptron(object):
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        sigFun = self.sigmoidActivation(inActs)
        return sigFun*(1 - sigFun)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        copyinAct = [1]
        for act in inActs:
            copyinAct.append(act)

        for Act in xrange(len(copyinAct)):
            self.weights[Act] = self.weights[Act] + \
                copyinAct[Act] * alpha * delta
            totalModification = totalModification + \
                abs(copyinAct[Act] * alpha * delta)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            pList = []
            for node in layer:
                pList.append(node.sigmoidActivation(currentList))
            outputList.append(pList)
            currentList = pList
            ...
            for layerNum in xrange(self.numHiddenLayers-1, -1, -1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    for nextNeuronNum in xrange(len(nextLayer)):
                        weightedDelta = nextLayer[nextNeuronNum].weights[neuronNum +
                                                                         1] * deltas[0][nextNeuronNum]
